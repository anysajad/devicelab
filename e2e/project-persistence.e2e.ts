...
import { fixtureUrl, gotoApp, previewFrames, waitForFrameReady } from './helpers';
...
test.describe('project persistence smoke', () => {
  test('seeded project restores workspace on boot', async ({ page, context }) => {
    ...
    await context.addInitScript((args: [string, string, string]) => {
      const [projectKey, lastProjectKey, recordJson] = args;
      localStorage.setItem(projectKey, recordJson);
      localStorage.setItem(lastProjectKey, JSON.stringify('smoke-restore-test'));
    }, [`${PROJECT_KEY_PREFIX}${projectId}`, LAST_PROJECT_KEY, seedData] as [string, string, string]);

    await gotoApp(page);

    // The workspace should show the seeded device
    const select = page.locator('nav[aria-label="Preview controls"]').first().getByLabel('Select device');
